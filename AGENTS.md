...
创建需求前，Orchestrator 必须先判断改动属于以下哪种范围：用户已明确指定的 App、可能匹配的已有 App、需要新建的 App，或仓库公共能力。

```text
识别仓库改动
  ↓
用户是否已明确指定 apps/<app>？
  ├─ 是且无冲突 → 直接使用该 App 的需求目录，不重复询问
  └─ 否或存在歧义 → 扫描 apps/ 中的已有 App
                       ↓
                     是否存在明显匹配？
                       ├─ 是 → 只推荐最匹配的一个 App，并询问用户确认
                       └─ 否或用户否认匹配 → 判断是否属于仓库公共能力
                                                ├─ 是 → 使用仓库级需求目录
                                                └─ 否 → 按新 App 启动流程处理
```

扫描与确认过程必须遵守第 13.4 节“所有 Agent 单轮单问”：每轮只确认一个独立决策点。不得一次列出多个候选 App 要求用户同时选择。
...
当已有 App 均不匹配且需求不属于仓库公共能力时，按以下流程启动新 App：

1. 按第 13.4 节“所有 Agent 单轮单问”访谈用户，先询问新 App 要解决的核心问题；信息不足时逐轮继续澄清。
2. 信息足以命名后，提出一个 `NNN-english-kebab-case-name` 格式的目录名并取得用户确认；名称必须使用英文 kebab-case。
3. `NNN` 为三位递增序号。只统计 `apps/` 下以合法三位数字开头且紧跟连字符的目录，取其中最大数字加一；不得按目录数量推算，异常命名目录不参与计算。
4. 用户确认目录名之前，不得创建新 App 目录或其需求文件。
5. 用户确认后，允许直接创建 `apps/<NNN-name>/`、`apps/<NNN-name>/docs/requirements/README.md` 和该 App 的第一份需求目录，无需先创建仓库级临时需求。此条是“先创建需求目录”的新 App 首次建档例外，不改变需求确认 Gate。
6. 第一份需求目录必须立即从全局模板复制 `requirement.md`、`technical-design.md` 和 `change-list.md`，并在 App 级需求总清单登记为 `Proposed`；后续访谈结果持续同步到这些文档。
7. 首次初始化只允许创建 App 目录与上述需求文档结构，不得生成应用代码、代码骨架、依赖或构建配置。完成需求方案、技术方案、改动范围并取得用户确认后，方可进入正常实施。
...
新 App 完成首次建档后，后续所有属于该 App 的需求都必须在 `apps/<app>/docs/requirements/` 下创建，并登记到该 App 的需求总清单；不得回流到仓库级 `docs/requirements/`。
...
以下规则适用于 Orchestrator 和所有子智能体，并覆盖需求访谈、需求澄清、方案确认、阻塞处理及其他所有向用户提问的场景：

1. 每轮最多向用户提出一个独立问题；提出后必须等待用户回答，再继续询问下一题。
2. 不得通过编号列表、并列分句、复合句或多个问号，把多个独立问题或决策点包装成一个问题。
3. 存在多个待确认项时，按重要性和阻塞程度排序，每轮只询问当前最关键的一项，其余问题留待后续轮次。
4. 提问前可以提供理解当前问题所必需的简短上下文，但上下文不得夹带额外问题。
5. 单个问题可以提供互斥选项；提供选项时必须明确标出推荐项，并简述推荐理由。
6. 子智能体需要用户决策时，只向 Orchestrator 上报当前最关键的一个问题，由 Orchestrator 按本节规则向用户询问。
